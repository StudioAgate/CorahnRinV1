...
if ($char_id) {
	$char = $db->row('SELECT %char_id,%char_name,%%characters.%char_content FROM %%characters WHERE %%characters.%deleted_at IS NULL AND %char_id = ?', $char_id);
    if (!$char) {
    ...
    $usersModifiersIds = array_reduce($modifications?:[], static function($result, $charmod) {
