...
$output = '
    ...
    .'<span class="ib charexp" style="cursor: default;">XP</span>'
...
foreach ($characters as $c) {
    if (strlen($c['char_name']) > 2) {
        $xp = preg_replace('~^.*"experience":\{"total":(\d+),.+$~isUu', '$1', $c['char_content']);
        $xp = is_numeric($xp) ? ((int) $xp) : '&ndash;';
        $anchor =
            '<span class="ib charid">'.$c['char_id'].'</span>'
            .'<span class="ib charname">'.$c['char_name'].'</span>'
            .'<span class="ib charexp">'.$xp.'</span>'
            .'<span class="ib charjob">'.($c['char_jobname'] ?: ' ('.tr('Personnalisé', true).') '.$c['char_job']).'</span>'
