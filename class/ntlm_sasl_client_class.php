...
class ntlm_sasl_client_class
{
	...
	{
		for($unicode= '', $a=0, $l=strlen($ascii); $a<$l; $a++)
	...
	public Function NTLMResponse($challenge, $password)
	{
		...
		for($response= '', $third=0; $third<21; $third+=7)
		{
			for($packed= '', $p=$third; $p<$third+7; $p++)
				$packed.=str_pad(decbin(ord($padded[$p])),8, '0',STR_PAD_LEFT);
			for($key= '', $p=0,$l=strlen($packed); $p<$l; $p+=7)
			{
				$s=substr($packed,$p,7);
				$b=$s.((substr_count($s, '1') % 2) ? '0' : '1');
				$key.=chr(bindec($b));
			}
			$ciphertext=mcrypt_encrypt(MCRYPT_RIJNDAEL_128,$key,$challenge,MCRYPT_MODE_ECB,$iv);
			$response.=$ciphertext;
		}
		return $response;
	}
