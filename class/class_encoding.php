...
/**
 * @param the $string to get to utf8
 * @global class Encoding
 * @global class unicode_replace_entities()
 */
function get_utf8 ($string) {
	$string = str_replace('&amp;', '&', $string);
	$encode = new Encoding;
	$utfstring = $encode->toUTF8($string);
	$utfstring = htmlentities($utfstring, ENT_COMPAT, 'utf-8');
	if (!preg_match('#^&.*$#isU', $utfstring)) {
		$oUnicodeReplace = new unicode_replace_entities();
		$utfstring = $oUnicodeReplace->UTF8entities($utfstring);;
	}
	$utfstring = preg_replace('/&amp;([a-zA-Z0-9#]{2,7});/', '&$1;', $utfstring);
	return $utfstring;
}
