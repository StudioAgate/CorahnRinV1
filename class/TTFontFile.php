...
class TTFontFile
{
    ...
    public function getMetrics($file)
    {
        ...
        if (!$this->fh) {
            throw new \RuntimeException('Can\'t open file '.$file, E_USER_ERROR);
        }
        $this->_pos = 0;
        $this->charWidths = '';
        $this->glyphPos = [];
        ...
        if ($version == 0x4F54544F) {
            throw new \RuntimeException('Postscript outlines are not supported', E_USER_ERROR);
        }
        if ($version == 0x74746366) {
            throw new \RuntimeException('ERROR - TrueType Fonts Collections not supported', E_USER_ERROR);
        }
        if (!in_array($version, [0x00010000, 0x74727565], true)) {
            throw new \RuntimeException('Not a TrueType font: version='.$version, E_USER_ERROR);
        }
    ...
    public function get_table($tag)
    {
        ...
        if ($length == 0) {
            throw new \RuntimeException(
                'Truetype font ('.$this->filename.'): error reading table: '.$tag, E_USER_ERROR
            );
        }
        fseek($this->fh, $pos);
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function extractInfo()
    {
        ...
        if ($format != 0) {
            dump(debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT));
            throw new \RuntimeException('Unknown name table format '.$format, E_USER_ERROR);
        }
        ...
        for ($i = 0; $i < $numRecords; $i++) {
            ...
            if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) { // Microsoft, Unicode, US English, PS Name
                ...
                if ($length % 2 !== 0) {
                    throw new \RuntimeException('PostScript name is UTF-16BE string of odd length', E_USER_ERROR);
                }
                $length /= 2;
                $N = '';
                while ($length > 0) {
        ...
        if (!$psName) {
            throw new \RuntimeException('Could not find PostScript font name', E_USER_ERROR);
        }
        $this->name = $psName;
        if ($names[1]) {
        ...
        if ($glyphDataFormat != 0) {
            throw new \RuntimeException('Unknown glyph data format '.$glyphDataFormat, E_USER_ERROR);
        }
        ...
        if (isset($this->tables['OS/2'])) {
            ...
            if ($fsType == 0x0002 || ($fsType & 0x0300) != 0) {
                throw new \RuntimeException(
                    'ERROR - Font file '.$this->filename.' cannot be embedded due to copyright restrictions.',
                    E_USER_ERROR
                );
        ...
        if ($metricDataFormat != 0) {
            throw new \RuntimeException('Unknown horizontal metric data format '.$metricDataFormat, E_USER_ERROR);
        }
        $numberOfHMetrics = $this->read_ushort();
        if ($numberOfHMetrics == 0) {
            throw new \RuntimeException('Number of horizontal metrics is 0', E_USER_ERROR);
        }
        ...
        if (!$unicode_cmap_offset) {
            throw new \RuntimeException(
                'Font ('.$this->filename.') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)',
                E_USER_ERROR
            );
        }
    ...
    public function getLOCA($indexToLocFormat, $numGlyphs)
    {
        ...
        if ($indexToLocFormat == 0) {
            ...
            if ($indexToLocFormat == 1) {
                ...
                throw new \RuntimeException('Unknown location table format '.$indexToLocFormat, E_USER_ERROR);
            }
        }
    }
